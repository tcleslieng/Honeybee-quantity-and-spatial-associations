...
meanpoints2$position <- ifelse(meanpoints2$number == "Large", 1.2, 2.2)  # Set the exact position of geompoints to be overlayed on the bar graphs
...
exp1.UD <- ggplot(R2, aes(x = number, y = proportion, fill = Direction)) + # ggplot structure
  ...
  scale_fill_manual(values = cbPalette) +
  geom_point(data = meanpoints2, aes(x = position, y = proportion, fill = direction), position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.02, dodge.width = 0.9), show.legend = FALSE, size = 2.2, colour = "azure4", na.rm = FALSE) + 
  theme(panel.background = element_blank(),
        axis.line = element_line(colour = "black"),
        axis.text = element_text(size = 14),
        legend.title = element_text(size = 14),
